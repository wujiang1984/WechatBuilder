<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="order_edit.aspx.cs" Inherits="WechatBuilder.Web.admin.order.order_edit" ValidateRequest="false" %>
  ...
  <dl>
    ...
    <dd>
      <table border="0" cellspacing="0" cellpadding="0" class="border-table" width="98%">
        <tr>
          <th width="20%">支付方式</th>
          <td><%=new WechatBuilder.BLL.payment().GetTitle(model.wid.Value,model.payment_id) %></td>
        </tr>
        <tr>
          <th>配送方式</th>
          <td><%=new WechatBuilder.BLL.express().GetTitle(model.express_id) %></td>
        </tr>
        <tr>
          <th>用户留言</th>
          <td><%=model.message %></td>
